                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {currentItems.map((course) => (
                        <CourseCard
                            key={course._id}
                            course={course}
                            onEdit={hasPermission(user, 'COURSE', 'update') ? openEditCourseModal : null}
                            onDelete={hasPermission(user, 'COURSE', 'delete') ? (course) => openDeleteModal(DELETE_TYPE.COURSE, course._id, course.name) : null}
                            onView={openViewCourseModal}
                            onManage={hasPermission(user, 'COURSE', 'update') ? openManageBranchesModal : null}
                        />
                    ))}
                </div>
            )}
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {currentItems.map((course) => (
                                    <CourseTableRow
                                        ...
                                        course={course}
                                        onEdit={hasPermission(user, 'COURSE', 'update') ? openEditCourseModal : null}
                                        onDelete={hasPermission(user, 'COURSE', 'delete') ? (course) =>
